...
class SysRoleServiceImplTest {
    ...
    void testDeleteRoleByIds_ContainsAdmin() {
        ...
        try (var mockedSecurityUtils = mockStatic(com.zhi.common.utils.SecurityUtils.class);
             var mockedSysUser = mockStatic(com.zhi.common.core.domain.entity.SysUser.class)) {
            // Mock getUserId() 返回管理员用户ID
            mockedSecurityUtils.when(com.zhi.common.utils.SecurityUtils::getUserId).thenReturn(1L);

            // Mock SysUser.isAdmin() 返回 true（是管理员）
            mockedSysUser.when(() -> com.zhi.common.core.domain.entity.SysUser.isAdmin(1L)).thenReturn(true);

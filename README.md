...
- **友链申请更安全**：申请页接入图形验证码（与登录同一开关），并在后台新增「显示友链申请入口」开关，
  关闭后前台隐藏入口、申请页给出提示、接口直接拒绝
- **留言板**：独立于文章的留言页 `/blog/guestbook`，匿名可提交（IP 限流 + 图形验证码 + 长度校验），
  并带输入时限（验证码签发到提交不足 3 秒直接拒绝、同一访客留言间隔 60 秒，提交按钮倒计时）；
...
- **配色可读性**：默认主题与 mo-blog 主题的浅色/深色四种组合逐页校验文字对比度
  （前台 13 条路由 × 811 个文本元素全部达到 WCAG AA）；主按钮/分页/plain 按钮/主色文字
  改用按对比度推导的配对前景色（`--mo-on-primary` / `--mo-primary-text`），Element Plus
  偏浅的 secondary/info/error 默认色在浅色下另行覆盖
...
- **功能开关全链路生效**：所有开关统一判定口径（前端 `isFeatureEnabled` / 后端 `BlogSwitchUtils`），
  评论/点赞/搜索关闭后前台隐藏入口**且接口拒绝**；页脚/版权/侧边栏/友链等展示类开关在所有前台页面
  一致生效（BlogLayout 自行加载设置）；浏览统计、评论审核、邮件通知、防盗链均在后端生效
  （防盗链拦截返回真实 403，不再被 401 处理器包装成 200）
- **每日 PV/UV 聚合**：按访客（IP/用户）去重计数，定时汇总，文章统计页新增近 30 天曲线
- **访问明细（PV/UV 明细）**：每次内容访问落一行明细（文章 + 自定义页面），记录访客标识/IP/UA/来源，
  标记是否当日独立访客；后台可按日期/类型/IP/关键字/仅独立访客筛选、看排行与区间汇总、导出与清理
  （默认保留 90 天，每天 01:10 自动清理）
- 统计页移除假数据兜底（接口失败显示错误/空态）
- 文章**评论数与真实评论联动**（发布/审核/编辑/删除路径同步），并提供历史一次性重算

### 工程与质量

- 删除死代码与孤儿依赖（BlogNav/BlogFooter、TinyMCE 整栈、旧 views/system 页面、无引用组件、
  22 个零调用前端 API 函数、TestController、druid 与表单构建死页面等）
- 评论/分类/标签导出按钮指向真实接口；修复若干统计假数据
- 修复分类总览页因响应字段取错（读 `rows` 而接口返回 `data`）导致的「暂无分类」
- 前端容器内 Vite 改用轮询监听（`DOCKER=true` 时开启），避免宿主机改动收不到
  inotify 事件、dev server 一直返回旧模块而“改了代码页面没变化”
- 各模块单测与守卫测试同步增强（含深浅色 `--mo-*` 色阶约定、主色配对前景色的断言）
...
- `blog_upload`（媒体库）、`blog_article_like` / `blog_comment_like`（点赞）、`blog_article_daily_stats`（每日 PV/UV）、`blog_bookmark`/`blog_notification`、`blog_message`（留言板）、`blog_page`（自定义页面）、`blog_visit_log`（访问明细）等表
- `blog_comment.like_count`、`blog_article.publish_time/format/content_md`、`blog_friend_link.email` 等列
- 新菜单与按钮权限（媒体管理、留言管理、页面管理、访问明细等，含角色 1/2 分配）、
  `email_notify_enabled`、`friend_link_apply_enabled` 等设置种子
...
- **友链申请**（图形验证码，可由后台开关控制是否开放）
- 站内通知铃铛、个人中心、移动端汉堡导航
- 主题（浅色/深色/跟随系统 + mo-blog 主题）、功能开关生效；两种主题的浅色/深色文字均按 WCAG AA 校验
...
- **媒体库**（上传记录检索、删除联动文件）、**访问明细**（PV/UV 明细查询/排行/导出/清理）
...
- 访问明细（PV/UV 逐条日志 + 排行/导出/清理）、友链申请图形验证码与前台开关
- 留言板增加图形验证码与输入时限（最短填写 3 秒 + 同一访客 60 秒一条，提交按钮倒计时）
- 深浅色配色审计：默认主题与 mo-blog 主题在四种组合下逐页实测文字对比度，前台全部达到 WCAG AA
- 死代码/孤儿依赖清理、统计假数据与失效导出修复、认证页一致性
- 修复迁移脚本非幂等问题（分类/友链重跑重复插入）与若干失效路由/权限
- 修复分类总览页「暂无分类」、容器内 Vite 改动不生效（轮询监听）

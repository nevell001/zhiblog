...
interface AnimationElement extends HTMLElement {
  _animationObserver?: IntersectionObserver
}

interface AnimationConfig {
  enter: Array<{ opacity: number; transform: string }>
  options: {
    duration: number
    delay: number
    easing: string
...
const animateDirective: Directive<AnimationElement> = (el, binding) => {
  const animationType = (binding.value as string) || 'fade-in-up'
  const delay = 0
        ...
        if (entry.isIntersecting) {
          ...
          setTimeout(() => {
            ...
            if (animation) {
              ...
              el.style.opacity = to.opacity.toString()

...
| `deploy-assets.zip`（GitHub）/ `deploy-assets.tar.gz`（Gitee） | 部署配套：数据库脚本、nginx 配置、`.env` 示例、本指南 |
...
# Gitee 发行版为 deploy-assets.tar.gz 时用：mkdir -p deploy/tmp && tar -xzf deploy-assets.tar.gz -C deploy/tmp

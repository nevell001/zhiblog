...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加文章-标签关联"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const articleId = row.articleId || this.ids
      getTag(articleId).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改文章-标签关联"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除文章-标签关联编号为"' + articleIds + '"的数据项？').then(function() {
        return delTag(articleIds)
      }).then(() => {
        this.getList()

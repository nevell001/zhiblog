<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="friendLinkList" @selection-change="handleSelectionChange" :height="tableHeight" stripe border>
      ...
      <el-table-column label="URL" align="center" prop="url" :show-overflow-tooltip="true" min-width="180" class="hidden-sm-and-down" />
      <el-table-column label="Logo" align="center" prop="logo" width="80" class="hidden-xs-and-down">
      ...
      <el-table-column label="描述" align="center" prop="description" :show-overflow-tooltip="true" min-width="150" class="hidden-md-and-down" />
      <el-table-column label="排序" align="center" prop="sort" width="70" class="hidden-sm-and-down" />
      ...
      <el-table-column label="创建时间" align="center" prop="createTime" width="160" class="hidden-sm-and-down">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" width="100" fixed="right" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-tooltip content="修改" placement="top">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['blog:friendLink:edit']" size="small"></el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top">
            <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['blog:friendLink:remove']" size="small"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" :width="dialogWidth" append-to-body>
      <el-form ref="friendLinkRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="友链名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入友链名称" :maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item label="URL" prop="url">
          <el-input v-model="form.url" placeholder="请输入URL" :maxlength="200" show-word-limit />
        </el-form-item>
        <el-form-item label="Logo" prop="logo">
          <el-input v-model="form.logo" placeholder="请输入Logo URL" :maxlength="200" show-word-limit />
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="form.description" type="textarea" placeholder="请输入描述" :maxlength="200" show-word-limit :rows="3" />
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="form.sort" :min="0" :max="9999" placeholder="请输入排序号" controls-position="right" />
        </el-form-item>
...
import { parseTime } from '@/utils/date-time' // 导入时间解析工具
...
function submitForm() {
  ...
  proxy.$refs["friendLinkRef"].validate(valid => {
    if (valid) {
      const promise = form.value.id != undefined 
        ? updateFriendLink(form.value)
        : addFriendLink(form.value);
...
function handleStatusChange(row) {
  ...
  updateFriendLink(statusChange).then(() => {

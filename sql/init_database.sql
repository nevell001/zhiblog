...
BEGIN
    ...
    IF OLD.category_id != NEW.category_id THEN
        UPDATE blog_category SET article_count = article_count - 1 WHERE id = OLD.category_id;
        UPDATE blog_category SET article_count = article_count + 1 WHERE id = NEW.category_id;
    END IF;
END$$
...
CREATE TRIGGER `tr_article_delete` 
AFTER UPDATE ON `blog_article` 
FOR EACH ROW 
BEGIN
    -- 软删除时减少分类文章数
    IF OLD.del_flag = '0' AND NEW.del_flag = '1' AND NEW.category_id IS NOT NULL THEN
        UPDATE blog_category SET article_count = article_count - 1 WHERE id = NEW.category_id;
    END IF;
    
    -- 恢复时增加分类文章数
    IF OLD.del_flag = '1' AND NEW.del_flag = '0' AND NEW.category_id IS NOT NULL THEN
        UPDATE blog_category SET article_count = article_count + 1 WHERE id = NEW.category_id;
    END IF;
END$$
DELIMITER ;

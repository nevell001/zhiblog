...
- 发布：`publish@general_artifacts` 把后端 jar 和前端 dist 上传到制品库；`release@gitee` 插件自动创建仓库「发行版」（复用 `tagName` 指定的 tag，附件为后端 jar + 前端 dist 压缩包 + sql 初始化脚本 + nginx.conf + .env.example + 部署指南）。
- 部署：注释掉的骨架，配置方式见下。

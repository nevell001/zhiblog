...
**Current Version**: v1.3.6 (as defined in pom.xml)

**Recent Changes**:
- v1.3.5: Module rename (ruoyi-* → zhi-*), registration fix, version display
...
14. **Version Management**: Version defined in `pom.xml` (currently 1.3.6), sync with `application.yml` app.version for consistency. See `docs/VERSION_MANAGEMENT.md` for comprehensive version update instructions
...
- `npm run test:coverage` - Coverage report with enforced thresholds (scoped to `src/stores|utils|api|components` `.ts` logic files; targets ≥70% lines/statements, ≥75% functions, ≥55% branches)

...
- Security config validation reads Druid console credentials with a fallback chain (`spring.datasource.druid.stat-view-servlet.login-password` → `spring.datasource.druid.master.statViewServlet.login-password` → env `DRUID_PASSWORD`), because this repo nests the block under `master`. Reading only the canonical path made dev warn spuriously and **blocked production startup** whenever only `DRUID_PASSWORD` was set.

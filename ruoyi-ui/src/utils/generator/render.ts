...
const componentChild: Record<string, Record<string, RenderFunction>> = {
  'el-button': {
    default(_h, conf, key) {
      return conf[key]
    }
...
const componentSlot: Record<string, Record<string, ComponentSlotItem>> = {
  'el-upload': {
    tip: (h, conf, _key) => {
      if (conf.showTip) {
        return () =>

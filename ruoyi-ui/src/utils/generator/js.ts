 ...
 * @return {*}
 */
export function makeUpJs(conf: FormConf, type: string): string {
 ...
 * @return {*}
 */
function buildData(conf: FieldElement, dataList: string[]): void {
 ...
 * @return {*}
 */
function buildRules(conf: FieldElement, ruleList: string[]): void {
  if (conf.vModel === undefined) return
  const rules: string[] = []
  if (trigger[conf.tag!]) {
    if (conf.required) {
 ...
 * @return {*}
 */
function buildOptions(conf: FieldElement, optionsList: string[]): void {
  if (conf.vModel === undefined) return
 ...
 * @return {*}
 */
function buildProps(conf: FieldElement, propsList: string[]): void {
  if (conf.dataType === 'dynamic') {
    if (conf.props!.props && conf.valueKey !== 'value') {
      conf.props!.props.value = conf.valueKey
    }
    if (conf.props!.props && conf.labelKey !== 'label') {
      conf.props!.props.label = conf.labelKey
    }
    if (conf.props!.props && conf.childrenKey !== 'children') {
 ...
 * @return {*}
 */
function buildBeforeUpload(conf: FieldElement): string {
 ...
 * @return {*}
 */
function buildSubmitUpload(conf: FieldElement): string {

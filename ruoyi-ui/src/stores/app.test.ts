...
// Get the mocked module using import
import Cookies from 'js-cookie'
const mockCookies = vi.mocked(Cookies)
const mockGet = mockCookies.get
const mockSet = mockCookies.set
...
describe('App Store 测试', () => {
  ...
  describe('toggleSideBar action', () => {
    ...
    it('应该在侧边栏打开时设置 Cookie 为 1', () => {
      ...
      appStore.toggleSideBar(false)
      expect(mockSet).toHaveBeenCalledWith('sidebarStatus', 1)
    })

    it('应该在侧边栏关闭时设置 Cookie 为 0', () => {
      appStore.sidebar.opened = true
      appStore.toggleSideBar(false)
      expect(mockSet).toHaveBeenCalledWith('sidebarStatus', 0)
    })
  ...
  describe('setSize action', () => {
    it('应该设置尺寸并保存到 Cookie', () => {
      ...
      expect(mockSet).toHaveBeenCalledWith('size', 'large')
    })
    ...
    it('应该支持所有尺寸选项', () => {
      ...
      sizes.forEach(size => {
        ...
        expect(mockSet).toHaveBeenCalledWith('size', size)
      })
    })
  })

...
watch(internalExpression, () => resolveExp())
function shouldHide(key) {
  return !(internalHideComponent.value && internalHideComponent.value.includes(key))
}
function resolveExp() {
  // 反解析 表达式
  if (internalExpression.value) {
    const arr = internalExpression.value.split(/\s+/)
    if (arr.length >= 6) {
      //6 位以上是合法表达式
      const obj = {
        second: arr[0],
...
onMounted(() => {
  internalExpression.value = props.expression
  internalHideComponent.value = props.hideComponent
})
</script>
